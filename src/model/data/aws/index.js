...
async function deleteFragment(ownerId, id) {
  ...
  const command_s3 = new DeleteObjectCommand(params_s3);

  // create a DELETE object command to send to DynamoDB
  const command_ddb = new DeleteCommand(params_ddb);

  try {
    // Use our client to send the command to S3
    await s3Client.send(command_s3);
    // USe our client to send the command to DynamoDB
    await ddbDocClient.send(command_ddb);

  } catch (err) {
    logger.error({err}, 'Error deleting fragment data or metadata in S3 or DynamoDB');
